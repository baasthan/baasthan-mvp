...
export const createPGSchema = z
  .object({
    ...
    securityDeposite: z.coerce
      .number()
      .gte(0, { message: "Enter a valid Amount" })
    ...
    singleSharingPrice: z.coerce
      .number()
      .gte(0, { message: "Enter a valid Amount" })
      .refine((val) => !Number.isNaN(val), { message: "Amount is required" })

      .optional(),
    doubleSharingPrice: z.coerce
      .number()
      .gte(0, { message: "Enter a valid Amount" })
      .refine((val) => !Number.isNaN(val), { message: "Amount is required" })

      .optional(),
    trippleShareingPrice: z.coerce
      .number()
      .gte(0, { message: "Enter a valid Amount" })
      .refine((val) => !Number.isNaN(val), { message: "Amount is required" })
      .optional(),
  ...
  .superRefine((args, ctx) => {
    if (args.availableOccupancyType.includes("singleSharing")) {
      if (!args.singleSharingPrice) {
        ctx.addIssue({
          path: ["singleSharingPrice"],
          message: "Rent must be greater than 0",
          code: z.ZodIssueCode.custom,
        });
      }
    }
    if (args.availableOccupancyType.includes("doubleSharing")) {
      if (!args.doubleSharingPrice) {
        ctx.addIssue({
          path: ["doubleSharingPrice"],
          message: "Rent must be greater than 0",
          code: z.ZodIssueCode.custom,
        });
      }
    }
    if (args.availableOccupancyType.includes("tripleSharing")) {
      if (!args.trippleShareingPrice) {
        ctx.addIssue({
          path: ["trippleShareingPrice"],
          message: "Rent must be greater than 0",
          code: z.ZodIssueCode.custom,
        });
      }
